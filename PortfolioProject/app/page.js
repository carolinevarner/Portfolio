          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className={`rounded-xl shadow-lg border hover:shadow-xl transition-shadow flex flex-col h-full ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`} style={{ backgroundColor: 'var(--nav-bg)', color: 'var(--nav-text)' }}>
              <div className="h-48 bg-gray-100 flex items-center justify-center overflow-hidden">
                <Image src="/project_photos/hackathon.jpg" alt="Hamster Helper - AI Hackathon Project" width={384} height={192} className="w-full h-full object-cover" />
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-2">
                  <h3 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Hamster Helper</h3>
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${isDarkMode ? 'bg-yellow-600/30 text-yellow-400 border border-yellow-600' : 'bg-yellow-100 text-yellow-800 border border-yellow-300'}`}>
                    🏆 1st Place
                  </span>
                </div>
                <p className={`text-base mb-4 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  A VS Code extension that scans active files for security vulnerabilities using AI. Features live color customization for severity levels, adjustable text size, light/dark mode toggle, and persistent settings. Built for the KSU AI Hackathon competition.
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  <span className={`px-3 py-1 rounded-full text-sm ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>TypeScript</span>
                  <span className={`px-3 py-1 rounded-full text-sm ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>VS Code Extension</span>
                  <span className={`px-3 py-1 rounded-full text-sm ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>OpenAI API</span>
                ...
                <a href="https://github.com/carolinevarner/AIHackathonKSU" target="_blank" rel="noopener noreferrer" className={`project-github-button px-6 py-3 rounded-lg transition-all duration-300 flex items-center justify-center mt-auto border ${isDarkMode ? 'bg-white text-black' : 'bg-black text-white hover:bg-white hover:text-black hover:border-black'}`}>
